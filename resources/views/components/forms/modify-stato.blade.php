    ...
    <div class="flex flex-row">
        <div class="w-10/12">
            <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST"
                  action="{{route('aggiornastato', ['id' => $stato['id']])}}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="stato_id" value={{$stato['id']}}>
                <div class="flex flex-row">
                    <div class="w-1/2">
                ...
                <div class="flex flex-row">
                    <div class="w-full">
                        ...
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               ...
                               placeholder="descrizione stato utente"
                               value="{{$stato['description']}}"/>
                        @error('username')
                        <div class="text-red-500 text-left text-sm">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <x-sidebar.menuDxRole menDx="modificaRuolo" roleid="{{$stato->id}}"></x-sidebar.menuDxRole>
    </div>
</main>
